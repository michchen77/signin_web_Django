...
def add_event(request):
    ...
    def add_event(request):
        ...
        try:
            Event.objects.create(id=eid,name=name,limit=limit,status=int(status),start_time=start_time)
...
def add_guest(request):
    ...
    resutl = Event.objects.get(id=eid).status
    if not result:
    ...
    try:
        ...
        return JsonResponse({'status': 10026, 'message': 'the event guest phont number repeat'})
